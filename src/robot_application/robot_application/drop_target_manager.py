...
class DropTargetManager:
    ...
    feasible candidates the one with the smallest arm alignment error is
    preferred; path length is used as a tiebreak.
    """

    def __init__(self, node):
        self.node = node

        self.node.declare_parameter('drop_target_manager_enabled', True)
    ...
    def select_navigation_goal(
        ...
        'Best' means smallest arm alignment error (closest to a perfect drop),
        with shortest Nav2 path as a tiebreak.
        """
        ...
        self.node.get_logger().info(
            "Drop target '%s' selected pose: x=%.3f, y=%.3f. Starting goal pose search."
            % (drop_id, float(normalized['x']), float(normalized['y']))
        )
